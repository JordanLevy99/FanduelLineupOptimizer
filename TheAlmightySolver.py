...
fix_these_names = {"T.J. Warren":"TJ Warren", "CJ McCollum":"C.J. McCollum", "Otto Porter Jr.":"Otto Porter", "DeAndre' Bembry":"DeAndre Bembry",
                   "J.J. Redick":"JJ Redick", "Kelly Oubre Jr.":"Kelly Oubre", "Larry Nance Jr.":"Larry Nance"}   # name.strip "." and strip "Jr" unless its tim hardaway
### TODO
    ...
    try:
        try:
            ...
            if e.errno != errno.EEXIST:
                raise
        shutil.move("/Users/jordanlevy/Downloads/nba-player.csv", "{}/{}/nba-player.csv".format(data_dir,date_getter()))    ### TODO Change this to your own directory
        print("File Moved")
...
def fanduel_maximizer(data, i):
    ...
    selected_players.to_csv('{}/{}/lineup-{}.csv'.format(data_dir, date_getter(), i), index=False)   # writes output to csv as lineup # .csv
    #selected_players = [i.replace(".","") for i in selected_players]

    print(selected_players)
    print('Total Salary: {}'.format(sum(selected_players['Salary'])))
    print('Total Projection {}'.format((sum(selected_players['Ceiling'])+sum(selected_players['Projection'])+sum(selected_players['Floor'])) / 3))
    ...
    for file in os.listdir('/Users/jordanlevy/Downloads'):
        if filename in file:
            ...
            try:
                ...
                if e.errno != errno.EEXIST:
                    raise
            shutil.move("/Users/jordanlevy/Downloads/{}".format(file), "{}/{}/{}".format(data_dir, date_getter(), file))    # Change this to your own directory
            #return files
...
def fd_data_handler(lineup, filename):
    fix_these_names = {"T.J. Warren":"TJ Warren", "CJ McCollum":"C.J. McCollum", "Otto Porter Jr.":"Otto Porter", "DeAndre' Bembry":"DeAndre Bembry",
                   "J.J. Redick":"JJ Redick", "Kelly Oubre Jr.":"Kelly Oubre", "Larry Nance Jr.":"Larry Nance", "Juancho Hernangomez": "Juan Hernangomez"}   # name.strip "." and strip "Jr" unless its tim hardaway
...
def players_list_name_fixer(filename):
    ...
    fix_these_names = {"T.J. Warren":"TJ Warren", "CJ McCollum":"C.J. McCollum", "Otto Porter Jr.":"Otto Porter", "DeAndre' Bembry":"DeAndre Bembry",
                   "J.J. Redick":"JJ Redick", "Kelly Oubre Jr.":"Kelly Oubre", "Larry Nance Jr.":"Larry Nance", "Juancho Hernangomez": "Juan Hernangomez",
...
def formula_generator(data, variable, row):
    return (round(data.loc[row, 'Ceiling'], 3) + round(data.loc[row, 'Projection']) + round(data.loc[row, 'Floor'], 3)) / 3 * variable
...
def main():
    ...
    if exclude_players:
        ...
        player_data = name_filter(player_data, excluded_players)  # Filters player data by players/teams given by excluded_players
    ...
    if len(fanduel_filenames) == 1:
        ...
        if ("entries" in fanduel_filenames[0]):
            player_index = 1
            entries_index = 0
        players_id = fd_data_handler(first_lineup_names, fanduel_filenames[player_index])   # handles the player list data, converts player name to player_id
    ...
    for i in range(len(first_lineup_names)):
        ...
        if len(fanduel_filenames) == 1:
            players_id = fd_data_handler(lineup, fanduel_filenames[player_index])   # handles the player list data, converts player name to player_id
