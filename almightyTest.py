...
def import_data():
    ...
    player_data = player_data.drop([2,4,6,7], axis=1)
    player_data.columns = ['Player Name', 'Salary', 'Position', 'Projection']
...
def fanduel_maximizer(data, i, max_proj=0):
    ...
    #print(decision_variables * data['Projection'].tolist())
    ...
    if max_proj > 0:
        prob += lpSum(proj_variables) <= (max_proj-0.01)

    prob += lpSum(salary_variables) <= 60000
    #print(type(salary_variables))
    ...
    selected_players.to_csv('Data/{}/lineup-{}-top10.csv'.format(date_getter(), i), index=False)   # writes output to csv as lineup # .csv
    print(selected_players)
    print('Total Salary: {}'.format(sum(selected_players['Salary'])))
    print('Total Projection {}'.format(sum(selected_players['Projection'])))
...
def fd_csv_enter(players_id, filemode):
    ...
    with open('Data/{}/FD_lineups-top10.csv'.format(date_getter()), mode=filemode) as FD_file:
...
def main():
    ...
    if len(fanduel_filenames) == 1:
        ...
        players_id = fd_data_handler(first_lineup_names, fanduel_filenames[1])   # handles the player list data, converts player name to player_id
        entries = fd_csv_edit_reader(fanduel_filenames[0])
    ...
    for i in range(10):
        ...
        if i == 0:
            players = fanduel_maximizer(player_data, i+1)
            ...
            if len(fanduel_filenames) == 1:
                ...
                fd_csv_enter(players_id, 'w')
                previous_proj = sum(players['Projection'])

                continue
            elif len(fanduel_filenames) == 2:
                players_id = fd_data_handler(lineup, fanduel_filenames[1])   # handles the player list data, converts player name to player_id
                entries = fd_csv_edit_reader(fanduel_filenames[0])
                fd_csv_edit(entries, players_id, 'w', 1)  # goes to csv edit
                fd_csv_enter(players_id, 'w')
                previous_proj = sum(players['Projection'])
            ...
            players = fanduel_maximizer(player_data, i+1, max_proj=previous_proj)
        previous_proj = sum(players['Projection'])
