...
def scrape_data(html, filename):
    ...
    if html is not None:
        ...
        #players_df['Salaries'] =
        columns = ['Name','Salaries','Team','Position','Opponent','Projected Minues','','','','','','DvP','DvPRank','O/U','Line','Total','Movement','','','','','Ceiling','Floor', 'Projection', 'Pts/$/K']
...
def download_projections():
    ...
    start_date = '11-22-2018'   # Start date of the 2018-19 NBA Season
    end_date = '4-12-2019'  # End date of the 2018-19 NBA Regular Season

    daterange = pd.Series(pd.date_range(start_date,end_date)).astype(str).apply(lambda x: x.split('-')).to_list()  # Creates a list of datetime objects
    ...
    for date in daterange:
        ...
        month = date[1]
        day = date[2]
        url = "https://rotogrinders.com/projected-stats/nba-player?site=fanduel&date={}-{}-{}".format(str(year), str(month).zfill(2), str(day).zfill(2))
        print(url)
